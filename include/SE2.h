...
public:
  ...
  inline const Map<Vec2T>& t() const { return t_; }
  inline const SO2<T>& q() const { return q_; }
  inline Map<Vec2T>& t() { return t_; }
  inline SO2<T>& q() { return q_; }
  ...
  {
    Mat3T Omega;
    Omega << SO2<T>::hat(omega.template block<1,1>(2,0)), omega.template block<2,1>(0,0),
             (T)0., (T)0., (T)0.;
    return Omega;
  }
